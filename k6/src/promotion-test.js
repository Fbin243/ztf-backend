...
export function collectPromotion() {
  const collectPromotionUrl =
    config.promotionUrl + `/api/v1/promotions/collect/${promotionId}`;
  ...
  logger.info(
    `Promotion ${promotionId} collected for user ${userId}: ${resp.status} - ${resp.body}`
  );
}
